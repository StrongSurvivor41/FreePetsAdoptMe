...
for (let i = 0; i < Items.length; i++) {
    document.querySelector('#load ul').innerHTML += `
        <div background="Bg.png" class="col-md-3 col-md-6">
...
document.querySelectorAll('.col-md-3.col-md-6 a').forEach(item => {
    item.onclick = () => {
        if (!item.classList.contains('activeCard')) {
            if (selectedSkinsAmount >= 2) {
                $('.errorMsg').fadeIn();
                document.querySelector('.formError').innerText = 'Maximum 2 pets!';
            } else {
...
function claim() {
    ...
    const statuses = ['Connecting to Rbxl.gz...', 'Authorizing User...', 'User Found!', 'Verifying Human...', 'Attempting Human Verification (1/2)...', 'Failed...', 'Attempting Human Verification (2/2)...', 'Error, Manual Verification Required To Claim Pets!'];
